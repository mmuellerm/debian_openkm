...
# For unknown reason I used version 2.5.1 of hsqldb in the container version 6.3.12. But the Tomcat-archiv from OpenKM contains only version 2.2.6.
# So I get an error while starting the container. The only solution is to download the required version of hsqldb and replace the jar file in the library directory of tomcat.
# The actual version from debian does not work because it needs a newer java version.
ADD https://www.hsqldb.org/repos/org/hsqldb/hsqldb/2.5.2/hsqldb-2.5.2.jar /usr/local/hsqldb-2.5.2.jar
...
RUN ln -s $OPENJDK_HOME $JAVA_HOME && \
    unzip /usr/local/tomcat.zip -d /usr/local/ && rm /usr/local/tomcat.zip && ln -s $CATALINA_HOME /opt/openkm && chmod u+x $CATALINA_HOME/bin/*.sh && mv /usr/local/hsqldb-2.5.2.jar $CATALINA_HOME/lib/hsqldb.jar && \
    ...
    sed -i 's|#hibernate.dialect=org.hibernate.dialect.HSQLDialect|hibernate.dialect=org.hibernate.dialect.HSQLDialect|' $TOMCAT_HOME/OpenKM.cfg && \
    sed -i 's|maxTotal="100" maxIdle="30" maxWaitMillis="10000" validationQuery="select 1"|maxTotal="100" maxIdle="30" maxWaitMillis="10000" validationQuery="select 1 from INFORMATION_SCHEMA.SYSTEM_USERS"|' $TOMCAT_HOME/conf/server.xml && \
    sed -i 's|username="sa" password="sa" driverClassName="org.h2.Driver"|username="sa" password="" driverClassName="org.hsqldb.jdbcDriver"|' $TOMCAT_HOME/conf/server.xml && \
    sed -i 's|url="jdbc:h2:${catalina.home}/repository/okmdb"/>|url="jdbc:hsqldb:${catalina.home}/repository/okmdb"/>|' $TOMCAT_HOME/conf/server.xml && \
